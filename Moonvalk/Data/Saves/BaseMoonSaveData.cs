...
namespace Moonvalk.Data
{
    ...
    public class BaseMoonSaveData<Unit> : IMoonSaveData
    {
        ...
        {
            _data = new System.Collections.Generic.Dictionary<string, Unit>();
        }

        /// <summary>
        /// A dictionary of all save data accessible by string names.
        /// </summary>
        private readonly System.Collections.Generic.Dictionary<string, Unit> _data;
        ...
        {
            var formattedData = JSON.Print(_data);
        ...
        {
            var result = (Dictionary)JSON.Parse(jsonData_)
                .Result;
#if (__DEBUG)
            ...
            var keys = result.Keys.Cast<string>()
                .ToArray();
            var values = result.Values.Cast<Unit>()
                .ToArray();
            for (var index = 0; index < keys.Length; index++)
            {
                ...
                var settings = _data.Keys.ToArray();
                foreach (var setting in settings)
                {
                    if (setting != keys[index])
                    {
                        continue;
                    }

                    _data[setting] = values[index];
                    break;
                }
            }
        }
        ...
        public BaseMoonSaveData<Unit> SetValue(params (string name_, Unit value_)[] pairs_)
        {
            foreach (var pair in pairs_)
            {
                ...
                if (_data.ContainsKey(formattedKey))
                {
                    _data[formattedKey] = pair.value_;
                    continue;
                }

                _data.Add(formattedKey, pair.value_);
            }
        ...
        public Unit GetValue(string name_)
        {
            ...
            if (_data.TryGetValue(formattedKey, out var value))
            {
                return value;
            }
